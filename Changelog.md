...
- [Feature] **Generic Per-Actor Resource Ledger (`cosmicvaultfaction.lua`):** `grantLedger(actor, factionIndex, ledgerKey, amount)` / `getLedger(...)` / `spendLedger(...)` — a shared "banked resource against a specific faction" primitive. `actor` is anything supporting `getValue`/`setValue` (a `Player()` for a per-player ledger, an `Alliance()` for one shared across an Alliance's members); `ledgerKey` namespaces the resource so multiple mods/mechanics can share one actor without key collisions. Built to replace Cosmic War's own hand-rolled Intel-banking pattern (`cw_intel_<factionIndex>`), which is now a thin wrapper around this.
- [Feature] **Generic Relief-Applied Tracker (`cosmicvaulteconomy.lua`):** `recordReliefApplied(scoreKey, factionIndex, amount)` / `getReliefApplied(...)` — a running, never-reset total generalizing a pattern Cosmic War built for its own Warbonds system: a payout that scales off how a tracked score changed during a holding period, which a player could otherwise game by personally applying the very relief action that would guarantee a good outcome. Any mod with a similar "snapshot a running total at the start of a hold, diff it at the end" need can share this instead of hand-rolling its own tracker per scoreKey.
- [Feature] **Passive Decay Registry (`cosmicvaulteconomy.lua`):** `registerPassiveDecay(scoreKeyPrefix, amountPerHour, floor)` registers a score family that should passively drift toward a floor over time; `tickPassiveDecay(scoreKeyPrefix, factionIndex, timeStep)` applies one tick of that decay for one faction. Cosmic Vault has no Galaxy-attached background loop of its own, so a consuming mod's existing background script calls `tickPassiveDecay` from its own update cycle — this shares the registration and drift math, not the scheduling.
- [Feature] **Galactic Hostility Index Reader (`cosmicvaulteconomy.lua`):** `getGalacticHostilityIndex()` soft-reads a plain `Server()` value (`cw_galactic_hostility_index`) that Cosmic War now publishes as the sum of every AI faction's current War Heat — returns 0 if Cosmic War isn't installed or hasn't published one yet. Lets any Cosmic mod scale its own systems off "how much overall warfare is happening right now" without a hard dependency on Cosmic War, mirroring this file's existing `cw_bridge` soft-include for the same reason.
- [Feature] **Faction Dossier Tooltip Builder (`cosmicvaultuikit.lua`):** `buildFactionTooltip(data)` formats a consistent "=== Name ===\nTraits: ...\nRelation: ..." block from already-resolved plain data (name/traits/relationText/extraLines) — every Cosmic mod with a faction-facing tab currently hand-formats this same shape slightly differently. Client-only, matching this file's existing top-of-file guard; additive alongside every consumer's current formatting, not a required migration.
- [Feature] **Dossier Detail Panel (`cosmicvaultuikit.lua`):** `createDossierPanel(container, rect, opts)` builds a labeled key/value detail panel — a fixed pool of label/value (or label/progress-bar) rows inside a frame, refreshed via `:setData({title=..., rows={...}})` rather than recreated per refresh, the same reasoning `createSortableTable` already uses for its own row elements. Meant to pair with `createSortableTable`: select a row in the table, show its full detail in the panel alongside it. First real consumer is Cosmic War's Galactic Politics rebuild (its own single-faction "Dossier" sub-tab). Client-only.
- [Feature] **Extended Bulletin Builder (`cosmicvaultmission.lua`):** `createBulletinEx(opts)` — a superset of the existing `createBulletin()` adding `formatArguments` (dynamic reward-text substitution) and `onAccept` (an inline accept-time script), the two fields Cosmic War's own 25+ mission files need and the original signature doesn't support. Fully additive; `createBulletin()` is unchanged.
- [Feature] **Border Sector Finder (`cosmicvaultterritory.lua`):** `getBorderSectors(factionAIndex, factionBIndex, radius)` returns the sectors where two named factions' territories actually meet — any sector controlled by one whose immediate neighbor is controlled by the other. Scans a bounded square (default 15 sectors) centered on the midpoint of the two factions' home sectors rather than the whole galaxy; `Galaxy():getControllingFaction()` is a faction-map lookup, not a sector load, so the scan itself stays cheap, but it's bounded regardless. First real consumer is Cosmic War's new Frontlines mechanic.
- [Feature] **New Shared Faction Conflict Scoreboard (`cosmicvaultconflict.lua`, new file):** `recordEvent(factionAIndex, factionBIndex, weight)` / `getScore(...)` / `resetScore(...)` — a generic "who's winning this fight between two factions" combined-score primitive for any Cosmic mod (or a future Cosmic War mechanic) that wants one without needing per-category weighting rules. Deliberately NOT what Cosmic War's own War Score & Attrition (`getWarScore()`) runs on — that system needs asymmetric per-category weights with an independent cap on just its kills component, which a single combined score can't represent, so it keeps its own bespoke implementation in `cosmicwarbridge.lua` rather than risk re-deriving logic that was only just fixed this same pass.
...
### 🎨 Shared Assets
- [Asset] **New Shared Icon: `EclipseInterface.png` (`data/textures/icons/`):** Added for Cosmic Ascendancy's new "The Eclipse: Command Interface" UI window (`ca_eclipse_interface.lua`, v1.7.0), matching the existing convention that shared icon assets live here rather than in the consuming mod's own folder (`forge.png`, `fss.png`, `AscendantMatter.png`, `EclipseDatacore.png`, etc. are all already hosted here for the same reason). No code changes — purely a new binary asset, referenced by path from Cosmic Ascendancy's code the same way every other Cosmic Vault-hosted icon already is.

### ⭐ API Enhancements
- [Feature] **UI Kit API Added (`cosmicvaultuikit.lua`):** New shared player-window tab building blocks, purely additive, client-only. `CosmicVaultUIKit.createHeaderLayout(container, opts)` builds a two-row header (title/totals row, then controls) with clearance for an optional sortable-column button strip, owning the vertical-clearance arithmetic in one place instead of leaving it to per-caller pixel math — three Cosmic Overhaul tab files (`factory_overview_tab.lua`, `command_center_tab.lua`, `galacticpolitics_tab.lua`) had each independently shipped the same header/sort-strip overlap bug before this existed, since `UIHorizontalSplitter`'s single `margin` constructor argument sets a uniform four-side inset with no bottom-only clearance option (confirmed against the API docs); the new helper sets a small uniform margin for general inset, then adjusts `marginBottom` specifically to the strip's height (both properties confirmed independently settable and live-recomputed on `UIHorizontalSplitter`, per vanilla `entity/merchants/refinery.lua`'s own post-construction `marginTop`/`marginBottom` usage). `CosmicVaultUIKit.createSortableTable(namespaceTable, container, rect, columns, opts)` wraps `ListBoxEx` plus a generated sort-button strip from a `columns` array, replacing hand-written per-column sort-callback functions; it requires the calling script's own namespace table as its first argument because Avorion resolves a UI element's callback string against the calling script's own namespace, never against an included library's (confirmed via `factory_overview_tab.lua`'s `Tab.onSelectedFunction` resolving to `FactoryOverview.clientFetchDataFromGalaxy`) — the kit installs its dispatcher functions directly onto the caller's table with generated per-instance names rather than assuming a shared global pool would be reachable. `CosmicVaultUIKit.STATUS_COLORS`/`statusColorForPercent`/`statusColorForKeyword` canonicalize the green/amber/red/gray status palette, which had already drifted between Factory Overview and Command Center's own slightly different "status red" values.
- [Feature] **Settings Schema API Added (`cosmicvaultsettingsschema.lua`):** New schema-driven convenience layer over the existing `cosmicvaultplayersettings.lua` (unmodified). `CosmicVaultSettingsSchema.define(modId, schema)` returns a handle (`get`/`getAll`/`set`/`resetToDefaults`, server-only, matching `cosmicvaultplayersettings.lua`'s own restriction) driven by a `{key, default, type}` array, with real key/type validation on write instead of a mod hand-writing each setting's plumbing separately. Also adds `getLocal(key)`, a client-side-only read path that intentionally does NOT route through `cosmicvaultplayersettings.lua` — that file's own v3.5.0 fix documented a real client-side crash from calling `Player():getValue()` without a server guard (WIKI.md section 10), yet Cosmic Overhaul's shipped `resourcedisplay.lua` calls the equivalent bare `Player():getValue()` client-side with no guard and no reported crash across multiple releases. This discrepancy is not resolved by this addition — `getLocal` follows the proven, shipped pattern and documents the open question inline rather than silently picking a side.

### 🪲 Bug Fixes
- [Bugfix] **Mission Rewards Silently Failed to Pay Out (cosmicvaultmission.lua):** `CosmicVaultMission.completeMission` wrote `player.money = player.money + creditReward` directly — `Player.money` is a **read-only** engine property (confirmed against the API stubs), so every credit reward routed through this shared function was silently discarded regardless of the amount passed in. Affects all 15 Cosmic Chronicles story missions that use it (`aimission.lua`, `artifactdelivery.lua`, `bottanmission.lua`, etc.). This is the same class of bug the v3.0.5-era Chronicles changelog previously misdiagnosed in the opposite direction — that entry blamed `player:receive()` as "unstable" and replaced it with a raw property write, which is what actually broke payouts; Chronicles' own most recent sweep already reverted that mistake locally, and this fixes the shared root cause in Cosmic Vault. Now uses `player:receiveWithoutNotify("Mission Reward", creditReward, {})`, paired with the function's own existing `sendChatMessage` so the player still gets exactly one reward notification, not two. Also hardened both `completeMission` and `failMission`'s `player:removeScript(missionId)` call: every real caller passes a semantic id string (e.g. `"ai_mission"`), never an actual script path, so the call was a guaranteed no-op — harmless only because Chronicles' structuredmission.lua-driven missions clean up their own script lifecycle independently. Both now check `player:hasScript(missionId)` first, so the call still does its documented job for a caller that legitimately passes a real script path (e.g. a bulletin-board mission's own `scriptPath`), while remaining a safe no-op for id-string callers like Chronicles.
- [Bugfix] **Runaway Retry Spam on a Failed Buff Attach (cosmicvaultbuffs.lua):** A player-reported log showed `cosmicbuff.lua:110: attempt to call global 'callable' (a nil value)` repeating hundreds of times in minutes. Root cause is a still-unconfirmed engine-side issue with `include()` resolution specifically for entity scripts attached dynamically via `entity:addScript()` (see `Avorion_Modding_Codex.md`, "A repeating `include error` scoped to one workshop folder" — this player's server is now the third confirmed occurrence, and the failure does not self-heal once triggered for a session). `CosmicVaultBuffs.applyBuff` itself has no way to fix that engine-side failure, but every current caller (e.g. Cosmic Overhaul's `captainelitetraits.lua` Commodore trait, polled every 5 seconds per ship) follows a `refreshBuff` fails → `applyBuff` retries pattern with no backoff, so a single stuck attach compounded into one `entity:addScript()` attempt per affected ship, per buff type, every single poll cycle, indefinitely. `applyBuff` now tracks the last attach attempt per `(entity, buffId)` (in-memory only, mirrors the existing `_biasKeys`/`_baseMultiplierKeys` tracking pattern in this same file) and skips re-attempting within a 15-second cooldown; `terminateBuff` and a successful `refreshBuff` both clear the cooldown immediately so a legitimately-working or intentionally-removed buff is never held back by it. This does not fix the underlying engine-side `include()` failure — it caps the retry rate so one stuck buff can no longer flood the log or hammer `entity:addScript()`.

## [v3.5.0]

### ⭐ API Enhancements
- [Feature] **News API Upgraded (`cosmicvaultnews.lua`):** `CosmicVaultNews.publishArticle` now accepts and normalizes an optional `article.breaking` boolean (coerced to a real `true`/`false` rather than trusting an arbitrary truthy value), formally documented in the function's header comment alongside the existing `title`/`content`/`category` fields — Cosmic Chronicles' News Board reads this flag to show a dedicated banner and fire an immediate chat alert for galaxy-shaking events, and it's now a first-class, documented part of the schema rather than an undocumented convention. Also implemented `CosmicVaultNews.getPublishedNews()`, previously a stub that did nothing but explain in comments why it wasn't implemented — it now works correctly for server-side callers by delegating to `cosmicvaultnews_server.lua`'s own storage via `Galaxy():invokeFunction`, giving the API real read/write symmetry. Client-side callers still can't get server data this way (there's no synchronous client/server call in Avorion), and the function says so explicitly rather than silently returning nothing.
