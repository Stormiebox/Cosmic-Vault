...
| `cosmicvaultweather.lua` / `_server.lua` | Versioned environmental condition API and owner |
| `cv_environment_controller.lua` / `cv_environment_effect.lua` | Loaded-sector mechanics and presentation |
...
- **`cosmicvaultdata.lua`** — stores structured Lua tables on entities via `dkjson`, plus tag-based grouping and querying. Versioned `SetRecord`/`GetRecord` helpers add schema validation and distinct missing, corrupt, and unsupported-version results without changing `SetTable`/`GetTable`.
...
- **`cosmicvaultarsenal.lua`** — generates balanced custom `Weapon`/`InventoryTurret` objects. `GenerateTurret` retains its original generic behavior and does not guarantee `config.weaponType`. `GenerateTypedTurret` validates a vanilla-supported weapon type and uses vanilla's seeded typed generator when a real Laser, Bolter, Cannon, or other dispatched type is required.
- **`cosmicvaulteconomy.lua`** — reads live market data, tracks famine and galaxy hostility, and exposes registered price hooks. Persistent market events have explicit start/get/end/price-delta calls; same-source events refresh, distinct overlaps combine, expiry survives restarts, and the final event delta stays within vanilla's ±30% price-change range. `TriggerMarketEvent` remains compatible.
...
- **`cosmicvaultterritory.lua`** — owns contested-zone data and durable per-kind materialization queues. `QueueMaterialization`, `ClaimMaterialization`, `CompleteMaterialization`, `RetryMaterialization`, and `GetMaterialization` preserve exact coordinates, lease work to one claimant, retain verified tombstones, and expose conflicts or exhausted retries as repair states. `RequireMaterializationRepair` stops automatic repetition when another system may already have performed the side effect but its receipt is ambiguous. Existing `resolveSiege` and `expandToSector` calls remain compatible and enqueue through the same lifecycle; legacy pending strings are imported once as evidence rather than treated as current state.
- **`cosmicvaultmission.lua`** — builds and posts bulletin-board missions, with `failMission()` for UI-handled failure and `grantItemReward()` for physical item payouts. **v4.0.0** added `createBulletinEx(opts)`, a superset of `createBulletin()` supporting `formatArguments` and `onAccept`, fully additive.
...
- **`cosmicvaultanomalies.lua`**, **`data/scripts/entity/cv_anomaly_rift.lua`**, **`data/scripts/entity/cv_anomaly_wreck.lua`** — spawns persistent, interactive points of interest. Spatial Rift claims now persist a deterministic reward snapshot before loot materializes. Interrupted, corrupt, or unsupported claims become visibly repair-required and never replay an uncertain reward.
- **`cosmicvaultweather.lua`** / **`server/cosmicvaultweather_server.lua`** — versioned weather lifecycle API: `StartWeather`, `RefreshWeather`, `EndWeather`, `GetWeather`, `ListWeatherAt`, `GetWeatherSnapshot`, and presentation-only `RegisterWeatherType`. Conditions have deterministic source ownership, exact coordinates, exclusive stacking groups, expiry, bounded retries, and retained tombstones. The old `triggerStorm`, `clearStorm`, and `getWeatherAt` exports remain compatible.
- **`sector/cv_environment_controller.lua`** / **`entity/cv_environment_effect.lua`** — canonical loaded-sector mechanics and presentation. The controller reconciles only the active conditions at its coordinate, applies owned ship bonuses and Solar Flare damage on the server, and renders native sector problems, warnings, sounds, and transient particles on clients. The old weather controller, debuff, and UI scripts are migration shims and must not be attached by new code.
- **`cosmicvaultweatherdictionary.lua`** — serializable built-ins for Ion Storm, Solar Flare, Dark Matter Fog, and Rift Instability, plus local mechanics helpers. Atmospheric weather and Rift instability use independent stacking groups. External registrations are presentation-only.
...
- **`cosmicvaultrift.lua`** / **`server/cosmicvaultriftescalation_server.lua`** — receipted Guardian and Depth 50+ extraction reporting plus the Rift hazard facade. Escalation uses Guardian kills plus half the deep-extraction count. Swarm attacks are prepared and verified before counters are consumed; failed attachments retry, while an interrupted materialization becomes repair-required. Guardian observation is additive, and only the minimal deep-extraction mission-utility extension remains a DLC VFS collision.
...
- **`cosmicvaultuikit.lua`** — **new in v3.6.0.** Shared player-window tab building blocks: `createHeaderLayout` (two-row header with sort-strip clearance handled once, centrally, instead of the per-file pixel math that independently produced the same header/sort-strip overlap bug in three different Cosmic Overhaul tab files), `createSortableTable` (ListBoxEx + generated sort-button strip from a column array), and a canonical `STATUS_COLORS` green/amber/red/gray palette (Factory Overview and Command Center had already drifted to two slightly different "status red" values before this existed). Client-only. See MODDER_GUIDE.md section 31 for usage — note in particular that `createSortableTable` requires the caller's own namespace table as its first argument, since Avorion resolves UI callback strings against the calling script, not against an included library. **v4.0.0** added `buildFactionTooltip(data)`, formatting a consistent faction dossier block from already-resolved plain data, and `createDossierPanel(container, rect, opts)`, a labeled key/value detail panel (optionally rendering a row as a progress bar instead of text) meant to pair with `createSortableTable` — select a row, show its detail alongside. See MODDER_GUIDE.md section 31 for both.
- **`cosmicvaultconflict.lua`** — **new in v4.0.0.** A generic "who's winning this fight between two factions" combined-score primitive (`recordEvent`/`getScore`/`resetScore`), built after Cosmic War's own War Score & Attrition proved the idea out. Separate, general-purpose infrastructure — deliberately not what Cosmic War's own scoreboard runs on, since that needs asymmetric per-category weights a single combined score can't represent. See MODDER_GUIDE.md section 34.
- **`cosmicvaultsettingsschema.lua`** — **new in v3.6.0.** A schema-driven convenience layer over `cosmicvaultplayersettings.lua` (section 10): define a mod's settings once (`key`/`default`/`type`) instead of hand-plumbing each one through several places, with real key/type validation on write. See MODDER_GUIDE.md section 32 — its `getLocal` client-side path exists specifically because `cosmicvaultplayersettings.lua`'s own server-only restriction (documented above, in this same section) is contradicted by Cosmic Overhaul's shipped `resourcedisplay.lua`, which safely reads the equivalent value client-side. That discrepancy is flagged, not resolved, by this addition — treat it as an open question, not a solved one.
- **`cosmicvaultupgradecategories.lua`** — **new in v3.7.0.** Shared registry sorting an upgrade system script into Military/Civilian/Misc, the upgrade-system equivalent of vanilla's own `WeaponTypes.armedTypes`/`.unarmedTypes`/`.defensiveTypes` for turrets. Pre-registers all 25 vanilla-generatable systems (cross-referenced against vanilla's own `upgradegenerator.lua`); `registerCategory(scriptPath, category)` lets a mod register its own, `getCategory(scriptPath)` defaults unregistered scripts to Misc rather than dropping them, and `getScriptsOfCategory(category)` returns every script currently registered under a category. See MODDER_GUIDE.md section 33. Consumed by Cosmic Overhaul's category-split Equipment Dock tabs; Cosmic Starfall registers its own systems into it.
...
- **Rift DLC interoperability:** the Vault receipts global Rift Guardian kills and Depth 50+ successful extractions. Escalation is `guardianKills + deepExtractions * 0.5`; each point above 10 adds a 5% Xsotan swarm-dispatch chance, capped at 50%. Counters fall only after at least one attack attachment verifies. The Guardian hook is additive; only the extraction-success wrapper still shares a vanilla DLC path.
...
- **Shared Intel ledger (v4.0.0):** Cosmic War's Intelligence Network now banks Intel through `CosmicVaultFaction`'s generic ledger primitive (section 3) instead of a bespoke per-player key — the practical effect is that a player's own Player Alliance now shares one pooled Intel balance per scouted faction instead of every member tracking their own in isolation.
- **Galactic Hostility Index (v4.0.0):** Cosmic War publishes the sum of every AI faction's current War Heat to a plain, soft-readable value; `CosmicVaultEconomy.getGalacticHostilityIndex()` (section 28) lets any Cosmic mod scale its own systems off "how much overall warfare is happening right now" without depending on Cosmic War directly.
- **Relief-applied tracking (v4.0.0):** the pattern Cosmic War built to stop its own Warbonds payout from being gamed by a player's own relief actions is now a general Vault primitive (`CosmicVaultEconomy.recordReliefApplied`/`getReliefApplied`, section 28) — any mod with a similar "payout scales off a tracked score's change during a hold" mechanic can reuse it.
- **Faction Conflict Scoreboard (v4.0.0):** a new, general-purpose "who's winning this fight between two factions" primitive (`cosmicvaultconflict.lua`, section 34), built after Cosmic War's own War Score & Attrition proved the idea out — for any other Cosmic mod (or a future Cosmic War mechanic) that wants a shared conflict score without War's own specific weighting rules.
